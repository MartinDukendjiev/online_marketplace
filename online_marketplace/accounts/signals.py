...
def send_successful_registration_email(user):
    ...
    plain_message = strip_tags(html_message)  # removes tags and shows only the text of the HTML
...
@receiver(post_migrate)
def add_user_groups(sender, **kwargs):
    if kwargs.get('app') and kwargs.get('app').name == 'your_app_name':  # Проверка за приложението
        if kwargs.get('created_models'):  # Проверка за създадени модели
            superuser_permissions = Permission.objects.all()  # have all permissions
